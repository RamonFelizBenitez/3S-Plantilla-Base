...
const Turnos = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setData([]);
      Swal.fire('Error', 'Error al cargar turnos', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    { accessor: 'Descripcion', header: 'Descripción del Turno' }
  ];

  const handleOpenForm = (row = null) => {
    if (row) {
      setEditId(row.TurnoID);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editId 
        ? `/api/configuracion/turnos/${editId}` 
      ...
      Swal.fire('Éxito', editId ? 'Turno actualizado' : 'Turno creado exitosamente', 'success');
      fetchData();
      handleCloseForm();
    } catch (err) {
      Swal.fire('Error', 'Error al guardar turno', 'error');
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`/api/configuracion/turnos/${id}`, {
        method: 'DELETE'
      });
      ...
      <DataTable 
        ...
        renderActions={(row) => (
          <button 
            ...
            onClick={() => handleDelete(row.TurnoID)}
