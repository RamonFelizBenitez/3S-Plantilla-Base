...
const Ausencias = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [resAusencias, resEmpleados, resTipos, resClasificaciones] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.EmpleadoID || !formData.TipoAccionID || !formData.FechaDesde || !formData.FechaHasta || formData.CantidadHora === '') {
      Swal.fire('Atención', 'Complete los campos obligatorios', 'warning');
      return;
    }

    try {
      if (formData.AusenciaID) {
        await axios.put(`/api/ausencias/${formData.AusenciaID}?empresaId=1`, formData);
        Swal.fire('Éxito', 'Registro actualizado', 'success');
      } else {
        await axios.post('/api/ausencias?empresaId=1', formData);
  ...
  const handleActionClick = (e, row) => {
    ...
    setMenuOpenId(row.AusenciaID === menuOpenId ? null : row.AusenciaID);
  };

  const handleAction = async (action, id) => {
    try {
      if (action === 'aprobar' || action === 'desaprobar') {
        const confirmText = action === 'aprobar' ? 'Se aprobará la Ausencia' : 'Se desaprobará la Ausencia';
        ...
        if (result.isConfirmed) {
          await axios.put(`/api/ausencias/${id}/status?empresaId=1`, { action });
  ...
  const handleTomaPosesionSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`/api/ausencias/${tomaPosesionData.id}/toma-posesion?empresaId=1`, tomaPosesionData);
  ...
  const handleEdit = async (row) => {
    ...
    setFormData({
      AusenciaID: row.AusenciaID,
      EmpleadoID: row.EmpleadoID,
      TipoAccionID: row.TipoAccionID,
      FechaDesde: formatDateStr(row.FechaDesde),
      FechaHasta: formatDateStr(row.FechaHasta),
      CantidadHora: row.CantidadHora || 0,
      ...
      FechaAprobado: row.FechaRegistro,
  ...
  const columns = [
    { accessor: 'AusenciaID', header: 'ID' },
    { accessor: 'EmpleadoNombre', header: 'Empleado' },
    { accessor: 'TipoAccionDesc', header: 'Tipo Acción' },
    { accessor: 'FechaDesde', header: 'Desde', render: row => formatDateStr(row.FechaDesde) },
    { accessor: 'FechaHasta', header: 'Hasta', render: row => formatDateStr(row.FechaHasta) },
  ...
  return (
    <>
      <DataTable 
        ...
        renderActions={(row) => {
          const isMenuOpen = menuOpenId === row.AusenciaID;
          return (
      ...
      <Modal 
        title={formData.AusenciaID ? "Editar Ausencia" : "Nueva Ausencia"} 
        isOpen={showModal} 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '25px' }}>
            ...
            <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                  <div>
                    ...
                    <select name="TipoAccionID" value={formData.TipoAccionID} onChange={handleChange} required disabled={formData.Aprobado} style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid #ccc' }}>
                      ...
                      {tiposAcciones.filter(t => t.TipoAccionID >= 60 && t.TipoAccionID <= 69).map(t => <option key={t.TipoAccionID} value={t.TipoAccionID}>{t.TipoAccionID} - {t.Descripcion}</option>)}
                    </select>
                  </div>
                  <div>
              ...
              <div style={{ marginBottom: '10px' }}>
                <h4 style={{ margin: '0 0 8px 0', color: '#334155', fontSize: '14px' }}>Datos de Ausencia</h4>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginTop: '10px' }}>
                  <div>
                    <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '13px' }}>Fecha Desde *</label>
                    <input type="date" name="FechaDesde" value={formData.FechaDesde} onChange={handleChange} required disabled={formData.Aprobado} style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid #ccc', height: '45px' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '13px' }}>Fecha Hasta *</label>
                    <input type="date" name="FechaHasta" value={formData.FechaHasta} onChange={handleChange} required disabled={formData.Aprobado} style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid #ccc', height: '45px' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold', fontSize: '13px' }}>Cant. Horas *</label>
                    <input type="number" name="CantidadHora" value={formData.CantidadHora} onChange={handleChange} required disabled={formData.Aprobado} style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid #ccc', height: '45px' }} />
                  </div>
                </div>
      ...
      <Modal 
        ...
        onSubmit={(e) => {
          ...
          const url = printType === 'imprimir_accion' 
            ? `/imprimir/ausencia/${printId}?f1=${f1}&f2=${f2}&f3=${f3}`
            : `/imprimir/carta_ausencia/${printId}?f1=${f1}&f2=${f2}&f3=${f3}`;
      ...
      {menuOpenId && (
        <>
          ...
          <div style={{
            ...
            {(() => {
              const row = ausencias.find(d => d.AusenciaID === menuOpenId);
              if (!row) return null;
              ...
              return (
                <>
                  <button style={btnStyle(canAprobar)} disabled={!canAprobar} onClick={() => { setMenuOpenId(null); handleAction('aprobar', row.AusenciaID); }}>
                    Aprobar
                  </button>
                  <button style={btnStyle(canDesaprobar)} disabled={!canDesaprobar} onClick={() => { setMenuOpenId(null); handleAction('desaprobar', row.AusenciaID); }}>
                    Desaprobar
                  </button>
                  {/* <button style={btnStyle(canProcesar)} disabled={!canProcesar} onClick={() => { setMenuOpenId(null); handleAction('toma_posesion', row.AusenciaID); }}>
                    Toma de Posesión
                  </button>
                  <button style={btnStyle(canAnular)} disabled={!canAnular} onClick={() => { setMenuOpenId(null); handleAction('anular', row.AusenciaID); }}>
                    Anular Acción
                  </button> */}
                  
                  <div style={{ height: '1px', background: '#e2e8f0', margin: '4px 0' }} />
                  
                  <button style={btnStyle(true)} onClick={() => { setMenuOpenId(null); handleAction('imprimir_accion', row.AusenciaID); }}>
                    Imprimir Acción
                  </button>
                  <button style={btnStyle(true)} onClick={() => { setMenuOpenId(null); handleAction('imprimir_carta', row.AusenciaID); }}>
                    Imprimir Carta
                  </button>
                </>
              )
