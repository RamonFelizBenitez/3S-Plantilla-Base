  ...
  useEffect(() => {
    const fetchPrintData = async () => {
      try {
        ...
        const vacacion = resAmonestaciones.data.find(a => String(a.VacacionID) === String(id));
        if (!vacacion) throw new Error('Vacación no encontrada');

        const empleado = resEmpleados.data.find(e => String(e.EmpleadoID) === String(vacacion.EmpleadoID));
        if (!empleado) throw new Error('Empleado no encontrado');

        const tipoAccion = resTipos.data.find(t => String(t.TipoAccionID) === String(vacacion.TipoAccionID));
        const clasificacion = resClasif.data.find(c => String(c.ClasificacionID) === String(vacacion.ClasificacionID));

        if (Array.isArray(resTipos.data)) {
          setTiposAcciones(resTipos.data.filter(ta => ta.TipoAccionID >= 50 && ta.TipoAccionID <= 70));
        }
        ...
        const printData = {
          Fecha: vacacion.FechaRegistro || vacacion.FechaInicio,
          AccionNumero: vacacion.VacacionID,
          Efectividad: vacacion.FechaInicio,
          ...
          AreaFuncional: empleado.DependenciaDesc || vacacion.DependenciaDesc,
          Cargo: empleado.CargoDesc || vacacion.CargoDesc,
          FechaIngreso: empleado.FechaIngreso,
          Sueldo: sueldoActual,
          TipoAccionID: vacacion.TipoAccionID,
    ...
    <div className="print-container">
          ...
          <div className="print-cell" style={{ borderRight: 'none', borderBottom: '1px solid #000', padding: 0, display: 'flex' }}>
            ...
            <div style={{ flex: 1, padding: '15px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <div style={{ marginBottom: '15px' }}>
                ...
                <div className="print-value" style={{ marginTop: '5px' }}>{formatDate(data.FechaInicio)}</div>
              </div>
              <div>
                <label style={{ fontSize: '12px', fontWeight: 'bold', color: '#666' }}>FECHA FIN</label>
                <div className="print-value" style={{ marginTop: '5px' }}>{formatDate(data.FechaFin)}</div>
              </div>
            </div>
          </div>
      ...
      <div className="print-row" style={{ flexDirection: 'column', flex: 1, borderBottom: 'none' }}>
        <div className="print-cell" style={{ borderRight: 'none', borderBottom: '1px solid #000' }}>
          <label style={{ fontSize: '13px' }}>10. VACACIONES</label>
