...
IF OBJECT_ID('dbo.NMCargos', 'U') IS NOT NULL DROP TABLE dbo.NMCargos;
...
IF OBJECT_ID('dbo.RHSolicitud', 'U') IS NOT NULL DROP TABLE dbo.RHSolicitud;
...
sql.connect(config)
    ...
    .then(res => { console.log('TABLES CREATED'); process.exit(0); })
    .catch(err => { console.error('ERROR CREATING TABLES:', err); process.exit(1); });
